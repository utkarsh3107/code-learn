...
public class MaximalConnectionList {
	
	private static Set<Integer> graph = null;
	private LinkedList<Integer> adjacencyList[];
	private int nodeElement;
	
	private MaximalConnectionList(int nodeElement)
    {
        this.nodeElement = nodeElement;
        this.adjacencyList = new LinkedList[nodeElement];
        for (int i=0; i<nodeElement; ++i)
	...
	private boolean[] BFS1(boolean[][] matrix, int N, int node, Queue<Integer> queue, boolean[] visited) {
		graph = new HashSet<>();
		visited[node] = true;
		queue.add(node);
		...
		while (queue.size() != 0) {
			...
			Integer value = adjacencyList[node].getFirst();
			
/*			for(LinkedList<Integer> eachElement : adjacencyList){
				
				
				if (!visited[eachElement]) {
					visited[eachElement] = true;
					queue.add(eachElement);
				}
			}*/
		}
		return visited;
